  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUpError) {
        ...
        // Automatically redirects to the sign-in page on successful signup (including when email verification is required).
        // Passes the email to auto-fill the field, and a success message.
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (data?.url) {
        ...
        // Keep loading true while popup is open, we'll navigate away on success anyway
      } else {
        setLoading(false);
      }
    } catch (err) {
